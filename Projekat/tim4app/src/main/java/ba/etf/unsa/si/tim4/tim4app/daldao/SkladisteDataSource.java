...
public class SkladisteDataSource {
	...
	public void update(int kapacitet, int kolicina)
	{
		String query = "UPDATE skladiste_plinskih_boca SET kolicina=? WHERE kapacitet=?";
		PreparedStatement ps = dbUtils.getPreparedStatement(query);
		if(ps == null) return;
	...
	public int getQuantityByCapacity(int capacity)
	{
		String query = "SELECT kolicina FROM skladiste_plinskih_boca WHERE kapacitet=?";
		PreparedStatement ps = dbUtils.getPreparedStatement(query);
		if(ps == null) return -1;
		try
		{
